...
function Welcome(props) {
	...
	const typeWriteItems = [
		...
		},
		{
			...
			itemDelay: 300,
			text:
				'Passionate about electrical and embedded systems engineering. I also dabble in web technologies!',
		...
		if (document.getElementById(typeWriteItems[0].htmlId) != null && doTypeAnim) {
			...
			if (document.getElementById(typeWriteItems[0].htmlId).innerHTML === '') {
				for (let i = 0; i < typeWriteItems.length; i++) {
					...
					let id = setTimeout(
						() => typeWriteAnim(element.htmlId, element.text, 0, element.charDelay),
						typeWriteDelay
					)
					timeoutIds.push(id)
